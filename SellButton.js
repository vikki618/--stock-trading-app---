...
class SellButton extends React.Component {
    ...
    handleSubmit = (id, currValue, maxQuantity, buyPrice) => {
        if (this.state.quantity == maxQuantity) {
            ...
            axios.delete(`${process.env.REACT_APP_BACKEND_URL}/state/company`, {
                data: { id },
    ...
    render() {
        return (
            <div className="field">
                ...
                <p className="help is-danger">{this.state.error && <p>{this.state.error}</p>}</p>
            </div>
        );
    }
}
...
const mapDispatchToProps = (dispatch) => ({
    ...
    addToMoney: sellValue => dispatch(SellAction(sellValue)),
    refresh: ({ id, companyDetails }) => dispatch(Refresh({ id, companyDetails }))
});
